...
- For an article in one of the ten **feature** categories, draw it: add an entry to BOTH `FEATURE_ICONS` (`apps/help/lib/featureIcons.tsx`) and `FEATURE_ENTITY_HEX` (`apps/help/lib/featureColours.ts`). All 172 feature cards have their own glyph, and `feature-icons.test.ts` keeps it that way — a card with no entry silently falls back to its category's glyph and looks like every other card in that category, which nothing at runtime can notice. [spec/55](specs/55-help-app.md) has the house style and the one rule that matters: draw what the card is _about_, and check the result doesn't land on a glyph the set already uses.
