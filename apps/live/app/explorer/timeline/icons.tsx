...
//
// Every event type gets its OWN glyph. A card shows it twice, small in
// the reason line and large in the preview box when there is no
// snapshot, so two kinds sharing a mark (a member joining and an action
// being assigned both wore user-plus) read as the same event at a
// glance. The path map is exhaustive over the closed type list, so a
...
// Heroicons outline paths (24-unit box), one per event type that had
// been sharing a mark with another.
const TAG =
...
const PENCIL_SQUARE =
  'M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L6.832 19.82a4.5 4.5 0 01-1.897 1.13l-2.685.8.8-2.685a4.5 4.5 0 011.13-1.897L16.863 4.487zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10';
const FOLDER_ARROW =
...
const SHARE_OUT =
  'M9 8.25H7.5a2.25 2.25 0 00-2.25 2.25v9a2.25 2.25 0 002.25 2.25h9a2.25 2.25 0 002.25-2.25v-9a2.25 2.25 0 00-2.25-2.25H15m0-3l-3-3m0 0l-3 3m3-3v11.25';
const TAKE_BACK = 'M9 15L3 9m0 0l6-6M3 9h12a6 6 0 010 12h-3';
const CHECK_BADGE =
...
const BELL =
  'M14.857 17.082a23.848 23.848 0 005.454-1.31A8.967 8.967 0 0118 9.75v-.7V9A6 6 0 006 9v.75a8.967 8.967 0 01-2.312 6.022c1.733.64 3.56 1.085 5.455 1.31m5.714 0a24.255 24.255 0 01-5.714 0m5.714 0a3 3 0 11-5.714 0M3.124 7.5A8.969 8.969 0 015.292 3m13.416 0a8.969 8.969 0 012.168 4.5';
const ENVELOPE_OPEN =
...
const SWATCH_X =
  'M4.098 19.902a3.75 3.75 0 005.304 0l6.401-6.402M6.75 21A3.75 3.75 0 013 17.25V4.125C3 3.504 3.504 3 4.125 3h5.25c.621 0 1.125.504 1.125 1.125v4.072M6.75 21a3.75 3.75 0 003.75-3.75V8.197M6.75 21h13.125c.621 0 1.125-.504 1.125-1.125v-5.25c0-.621-.504-1.125-1.125-1.125h-4.072M6.75 17.25h.008v.008H6.75v-.008zM15 4.5l4.5 4.5m0-4.5L15 9';

// One path per event type. `null` marks the one drawn by a component
// (the sidebar's two-person team mark) rather than a path. Exported so a
// test can check that no two types share a mark.
export const EVENT_GLYPH_PATHS: Record<KnownTimelineEventType, string | null> = {
  diagram_created: PLUS,
  diagram_edited: PENCIL,
  diagram_renamed: TAG,
  diagram_deleted: TRASH,
  diagram_duplicated: COPY,
  diagram_moved: FOLDER_ARROW,
  team_diagram_added: SHARE_OUT,
  team_diagram_removed: TAKE_BACK,
  diagram_offline: CLOUD_OFF,
  diagram_synced: CLOUD,
  diagram_opened_by_visitor: EYE,
  diagram_copied_by_visitor: CLIPBOARD_COPY,
  folder_created: FOLDER_PLUS,
  folder_deleted: FOLDER_MINUS,
  comment_added: CHAT,
  comment_resolved: CHECK_BADGE,
  action_assigned: CLIPBOARD_LIST,
  action_completed: CHECK,
  share_link_created: LINK,
  share_link_expiring: BELL,
  ...
  team_created: null,
  team_invite_received: ENVELOPE,
  team_invite_accepted: ENVELOPE_OPEN,
  team_invite_declined: X_CIRCLE,
  team_member_joined: USER_PLUS,
  team_member_left: LEAVE,
  team_member_removed: USER_MINUS,
  team_role_changed: SHIELD,
  team_renamed: PENCIL_SQUARE,
  team_deleted: ARCHIVE_X,
  team_invite_link_enabled: GLOBE,
  team_invite_link_disabled: LOCK,
  token_created: KEY,
  token_revoked: KEY_OFF,
  token_expiring: CLOCK,
  theme_saved: PALETTE,
  theme_deleted: SWATCH_X,
  image_uploaded: PHOTO,
};

export const EVENT_ICONS: Record<string, ReactNode> = Object.fromEntries(
  Object.entries(EVENT_GLYPH_PATHS).map(([type, d]) => [
    type,
    d === null ? <TeamIcon /> : <Glyph d={d} />,
  ]),
);
