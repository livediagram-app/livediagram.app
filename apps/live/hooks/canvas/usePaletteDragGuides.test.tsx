...
function dragOver(target: HTMLElement, clientX: number, clientY: number) {
...
describe('usePaletteDragGuides — insert between (spec/139)', () => {
  it('offers a slot when the drag hovers a gap on an event-storming board', () => {
    const { wrapperRef, result } = render({ canInsertBetween: true });
  ...
  it('puts the ghost in the slot, so the preview and the drop agree', () => {
    const { wrapperRef } = render({ canInsertBetween: true });
    dragOver(wrapperRef.current, 236, 100);
  ...
  it('never offers a slot on an ordinary board', () => {
    const { wrapperRef, result } = render({ canInsertBetween: false });
    dragOver(wrapperRef.current, 236, 100);
  ...
  it('closes the slot when the drag moves off the gap', () => {
    ...
    dragOver(wrapperRef.current, 236, 900);
    expect(getInsertionSlot()).toBeNull();
  });
  ...
  it('clears the slot when the drag ends', () => {
    const { wrapperRef, unmount } = render({ canInsertBetween: true });
    dragOver(wrapperRef.current, 236, 100);
    unmount();
    expect(getInsertionSlot()).toBeNull();
  });
  ...
  it('does not inherit the previous drag\u2019s slot', () => {
    const first = render({ canInsertBetween: true });
    dragOver(first.wrapperRef.current, 236, 100);
    expect(getInsertionSlot()).not.toBeNull();
    first.unmount();
    const second = render({ canInsertBetween: true });
    expect(getInsertionSlot()).toBeNull();
    // A fresh drag that never hovers a gap keeps it that way.
    dragOver(second.wrapperRef.current, 100, 100);
    expect(getInsertionSlot()).toBeNull();
  });
});
