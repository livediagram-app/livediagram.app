...
export const metadata = helpMetadata({
  title: 'Technology',
  description:
    'Full-colour AWS, Azure, Cloudflare, Firebase, and generic infrastructure brand icons for system-architecture diagrams.',
...
- **Generic infrastructure:** Kubernetes, Docker, PostgreSQL, MySQL, Redis, MongoDB, Cassandra, Kafka, RabbitMQ, Elasticsearch, Nginx, GraphQL, Node.js, React, GitHub, GitLab, Terraform, Prometheus, Vercel, Supabase.
...
<Figure caption="The Tech category shows a grid of full-colour brand tiles, each in its own accent hue with a caption beneath.">
  <TechnologyCatalogue />
</Figure>
...
<Step n={1}>
  Open the palette's category picker and choose **Tech**, in the **Decorate** band. (The category is
  labelled Tech for width; it is the Technology catalogue this page describes.)
</Step>

<Step n={2}>
  Pick a **provider** (AWS, Azure, Cloudflare, Firebase, Generic) to see its marks, or type in the
  search box to look across all of them at once.
</Step>

<Step n={3}>Use the breadcrumb at the top to go back to the provider list.</Step>
...
<Note>
  ...
  when you change the diagram's colour scheme.
</Note>
