...
That second check is a backstop rather than the main protection. The protection is that the editor only ever sends a value from a fixed list in the first place: where a value could come from something you named, it is mapped to a fixed token before it is sent. A colour scheme you built and named yourself is reported as simply "Custom", and your name for it never leaves your browser.
