...
It's purely a view setting: it changes nothing about the diagram, is never saved, and isn't shared, so each viewer tilts independently. It's available to everyone, including view-only visitors, since looking is read-only.
