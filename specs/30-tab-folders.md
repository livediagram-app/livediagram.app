...
- **Folder** entries render as a **container**, clearly distinct from the tab pills inside it: a **dashed** rounded boundary over a tinted inset surface, carrying a **folder glyph** (drawn closed when collapsed, open when expanded — no separate chevron, the folder icon itself signals the state), the folder **name** (small caps), and the member count as a small rounded **badge**. **Collapsed** = just that chip; **expanded** = the chip plus the inline member pills, which behave exactly like loose pills (selection, presence, drag-reorder, context menu).
