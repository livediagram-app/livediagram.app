...
**Table** (Tools accordion). Drops a 3×3 `table` element at the viewport centre. The grid divides the element box evenly; double-click a cell to edit its text (Enter / blur commits, Escape cancels, Tab / Shift+Tab walk cells). The first row renders as a header (`headerRow`, tinted band + bold) by default. `cells` is the row-major source of truth (`cells[r][c]`, kept rectangular by the helpers in `packages/diagram/src/table.ts`: addTableRow / removeTableRow / addTableColumn / removeTableColumn / setTableCell). Cell text commits through `onCommitCells` (the whole grid, mirroring how labels commit). When the table is selected, hovering a column's top gutter reveals a floating **add-left / delete / add-right** toolbar, and hovering a row's left gutter reveals **add-above / delete / add-below** — all handled inside the table component (delete is disabled at one row / column). The Colours accordion tints the grid lines (stroke), cell background (fill) and text; the whole table resizes via the normal element handles (per-column / per-row pixel sizing is a follow-up).
