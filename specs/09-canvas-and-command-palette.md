...
- **Avatar** — drops a single **circular image** placeholder (`createAvatar`: a square, aspect-locked image with a `full` corner radius + `cover` fit). Sits beside Image because it's one element, not a composite; double-click it to pick / upload a photo. Hidden when image upload is unavailable (same gate as Image).
...
- **Hero** — a large **image** with a title + supporting line on a themed **caption card** inset near the bottom (`createHero`: image + accent card + title + body). The card follows the theme and keeps the white text legible, while leaving the image reachable above it (see below).
...
The single-element **Avatar** (a circular image) is **not** a component — it lives in the **Tools** tab beside Image (see below), since it isn't a composite.

**Images.** Hero / Header carry an image (Avatar _is_ one); the image starts as an **empty placeholder** the user fills later by **double-clicking it** — we deliberately do **not** auto-open the picker on drop (it was jarring, popping a modal over the just-placed component). The Hero's caption card is inset rather than covering the whole image precisely so the image stays double-clickable. The image-bearing items (Hero, Header, and the Tools-tab Avatar) are **hidden when image upload is unavailable** (no R2 / view-role — same gate as the Image tool); the Banner always shows. A new `BorderRadius` value **`full`** (clamped to 50% → a circle on a square, a pill on a rectangle) backs the circular avatar, and a new `ImageElement.objectFit` (`'cover' | 'contain'`, default `contain`) lets the hero/avatar fill their box rather than letterbox. `ImageElementView` now honours both the element's `borderRadius` and `objectFit`.
