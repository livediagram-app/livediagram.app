...
- **Banner** — drops a decorative title block to head a diagram: an accent-filled rounded bar with a bold **title** and a muted **subtitle**. It is a **composite, not a new element type** — `createBanner` (in `packages/diagram/src/factories.ts`) returns a `square` shape + two `text` elements that share one `groupId`, so the banner moves / locks / copies as a unit yet stays fully editable (recolour the bar, retitle the text, or ungroup to break it apart). The bar is filled with the active tab theme's accent (`elementStroke`, falling back to brand sky on the Basic theme — mapped in `useElementCreation` so the diagram package stays theme-agnostic) and the text is white for contrast. Dropped at the viewport centre as one undo-able action via `addBoxedGroup`. **Desktop + mobile** both offer it (unlike the Spotlight _tool_). Building it from existing primitives means it inherits theming, resizing, fonts, format-painter, copy/paste, and export for free, touching none of the per-`type` switches a bespoke element would.

### Placement on add

The **Icon**, **Table**, **Annotation**, and **Banner** buttons place the new element at the **centre of the visible canvas viewport** (accounting for pan), auto-selected (the Banner selects its group). The draw-capable buttons (shape / text / sticky / image / arrow) instead arm the combined tap-or-drag gesture — see [Adding elements](#adding-elements--tap-to-drop-or-drag-to-draw) — where a tap drops it at the tap point and a drag sizes it.
