...
> ephemeral id for its broadcast presence / cursor (spec/61 §6), so the real
...
(spec/122) is client-claimed and relayed unchanged — the room's own comment
...
- **Unspoofable** without the room learning who anyone is: the token never
  reaches another peer, so spec/61 §6 stands untouched.
...
| All ordinary editing, import, export, themes, layout cleanup                                                                                             | Nothing to collide over that the selection lock (spec/07) doesn't already handle   |

### Two elements this changes

**Reveal zone (spec/106)** is per-viewer today: `revealedIds` is local state,
...
stays exactly as it is, per-viewer. That is a real change to spec/106 and needs
its spec amended alongside this one.

**Bring Focus (spec/144)** currently says, in as many words, that anyone in the
...
same act as "everybody stop and listen". With no facilitator, spec/144's rule is
unchanged. Spec/144 gets the amendment.
...
- **Offline diagrams** (spec/76) have no room, so no baton and no badge.
...
token. No user content, no names — spec/22's rule is unchanged.
