...
- **Tab**: Loaded (a tab's content was fetched for viewing — the first tab when a diagram opens (from the editor bootstrap) plus each tab switched to (`usePerTabLoad`); the search prefetch `loadAllTabs` deliberately does not emit); Created; Deleted; Duplicated; Renamed; Locked; Unlocked; Linked; Reordered; Imported `JSON`/`Markdown`; Aligned; Cleared; Changed `Font`/`DefaultTextSize`; Started `CountdownTimer`/`StopwatchTimer`/`Vote` (a live session tool launched on the tab, spec/39); Ended `Vote`; Revealed `Vote`.
