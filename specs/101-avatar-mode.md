...
- **Two looks**, `male` (short hair, trousers) and `female` (long hair, a skirt), drawn on the same skeleton so the walk / hop / flag animations are shared. **Right-clicking the character toggles between them** (a right-click anywhere else on the canvas does nothing — the context menu stays shut in this mode). It is a costume choice: nothing is persisted, and it carries no meaning beyond which sprite is drawn.
- **The shirt is the participant's presence colour** — the same colour as your cursor, your name chip, and your tab-bar dot — so on a shared diagram you can tell at a glance which character is whose. Falls back to the brand cyan when there is no participant colour.
...
- The packet is deliberately tiny — feet position, facing, look, walking flag, step frame, hop height, wave frame — and goes out at the **same ~30 Hz throttle as the cursor**, from the same gates (hydrated, a diagram id, and either a share link or a team diagram).
...
- **`apps/live/hooks/canvas/useAvatarWalk.ts`** — the state slice: position / target / facing / hop / wave / look, the `requestAnimationFrame` loop, the camera nudge, and the presence publisher.
...
- Any persistence of where the avatar was left, or of which look you picked.
