...
- Layer ids are **fixed sentinels** (`layer:template:<role>`, the `layer:default` pattern), not random uuids: re-applying the same template converges, and elements copied between two tabs cut from the same template keep their band.
...
- Shipped so far: **Kanban**, a "Board" layer (sprint title, lane containers, lane headers) under a "Cards" layer (card bodies, ticket text, priority chips).
