...
1. **A press ships the whole element, late.** An element change only leaves
   ...
   every receiver (spec/75). Two presses on one card inside that window
   overwrite each other.
2. **Collaborative presses pushed undo history.** `useCollabElements` wrote
   through `commitTabs`, which pushes an undo step, although spec/122 said it
...
- **A vote has a round** (spec/39). `TabVote.round` is minted at Start and
...
- `idea`: one anonymous card appended. No author, still (spec/125). The box
