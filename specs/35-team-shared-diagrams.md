...
- The WebSocket upgrade resolves `role = 'edit'` for a joined member of the diagram's team. The `o=` param was once trusted unverified (browsers can't set a Bearer on the upgrade), carrying the same forgeability the REST header fix above closed; the signed **room ticket** that was named here as the proper follow-up has since shipped — `POST /api/diagrams/<id>/room-ticket` mints a one-time ticket the client redeems on the upgrade (see [spec/11](11-api.md)), and team members are admitted by ticket only. The room still only relays ops (no persistence; the secured REST path is the only way to save).
...
- **Move to folder picker**: one ownership-aware modal for every diagram (personal or team), built on the **shared placement browser** (spec/15 + spec/76): a space overview first (Personal Space + each team), then that space's folder drill-down with a "here" card at every level and an inline New Folder tile. Picking a destination routes by scope: a personal folder files the diagram personally (moving a team diagram out transfers it to the mover, per the rules above), a team root/folder files it into that team. **Every diagram-move surface offers every space** — the /explorer page, the team page's own library, and the editor's floating Explorer panel (the panel's Current Diagram row includes team diagrams) — so a team diagram is never trapped: Personal Space and the other teams are one Back away on the space overview. Only **folder** moves stay scoped (a folder can't change scope: personal folders re-parent within the personal tree, team folders within their team). The team page supports a `&folder=<id>` deep-link param (read at mount) so search results and links can open a specific team folder.
- **Sidebar**: the personal "Personal Space" tree and each team in the Teams section render as the same collapsible folder tree (a team expands to reveal its folders). There is no separate "All diagrams" row — Personal Space lists Unsorted and the root folders directly. The in-editor floating Explorer panel's Teams accordion and its Current Diagram section give a team-library row a per-row **Delete** to any joined member (the api authorises owner-or-member delete). Fuller team-row management (rename, move, the full menu) stays on the /explorer + team pages. Because team rows aren't in the personal list the panel can't optimistically prune, the panel hides a just-deleted team row locally until the team-library sweep re-fetches.
- The sidebar's personal tree section is labelled **"Personal Space"** to contrast with team libraries. (It was "My Work" until 2026-09-21; the name now mirrors "Team Spaces", so the two kinds of library read as the same noun, and the help article moved from `explorer/my-work` to `explorer/personal-space`.)
