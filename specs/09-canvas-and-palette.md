...
**Sub-categories keep the tab count down.** When a category holds distinct-but-related families, it splits them within the one panel rather than spending a whole top-level tab on each. Two shapes exist: a lightweight always-visible uppercase heading (`PaletteSectionLabel`, used by the Favourites edit groups) for a panel with only a couple of short sections, and — where the sections multiply — **exclusive accordions**. The **Tools** tab uses the accordions: as the tool count grew to sixteen, the old flat grid stopped scanning (a pencil, a table, and an avatar are not one family), so the tab is themed groups, each a collapsible section holding a 3-column grid: **Write** (Text, Note, Annotation), **Draw** (the gesture tools — Pencil, [Highlighter](81-highlighter.md), [Polygon](84-polygon-tool.md), Arrow), **Structure** (Table, Frame, [Timeline](51-timeline-rail.md)), **Blocks** (the rich content cards — [Code block](82-code-block.md), [Checklist](83-checklist.md), [Link card](40-link-cards.md)), **People & media** (Image, Avatar, User), and **Data** (the charts from [spec/53](53-pie-chart.md) — pie, bar, line, progress, rating, folded in from what used to be a standalone Data category). **One group open at a time, the first (Write) open by default**; clicking another header switches, clicking the open one collapses it, so the tab stays one glance tall however many tools land. The header is the `PaletteSectionLabel` voice promoted to a chevroned button with the same animated collapse as the context-menu accordions (`PaletteAccordionSection` in `palette-create-tabs.tsx`); the open state is per-mount, deliberately not persisted. Group membership lives on the tile defs (`toolGroup` in `palette-tile-defs.tsx`, with the ordered `TOOL_GROUPS` catalogue beside it); tile **ids are unchanged**, so favourites persistence (spec/78) and the search panel are untouched by the grouping. Toggles emit `UI`/`Toggled`/`ToolGroup` telemetry (spec/22). **A search box sits above the accordions** (the same input + inline clear button as the Icons / Technology pickers, via the shared `PaletteSearchInput` primitive): the accordions keep the tab compact but hide most tiles, so typing filters by tile label / caption / description across **every group plus Data** and swaps the accordions for one flat grid of matches (with a "No tools match" empty state); clearing the query restores the accordions with their prior open group. The first keystroke per mount emits `UI`/`Searched`/`ToolSearch` (spec/22).
