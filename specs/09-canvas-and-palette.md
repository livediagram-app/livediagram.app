...
**Stickers category** (colour emoji, [spec/116](116-stickers.md)). Around 190 native colour emoji — the informal half of a board: reacting to somebody's work, showing how you feel, marking status, pointing at things, celebrating, decorating, meeting props, work objects, people, and a bit of fun. A sticker **is** an icon: the same `IconDef`, the same `icon` shape kind, the same add / drag / fold-into-shape / favourite / export paths, so nothing new is plumbed for it. It browses in ten disjoint groups (Reactions, Feelings, Status, Direction, Celebrate, Decorate, Meeting, Work, People, Fun — `STICKER_CATEGORIES` in `lib/stickers.ts`) with search across all of them ([spec/109](109-palette-category-browse.md)), and its tiles draw the glyph at 24px in a 4-column grid rather than the Icons grid's 18px in 5, because a sticker is a small picture rather than an outline. Ids keep their shipped `emoji-` prefix (`isStickerId` in `@livediagram/icons`), which is also what separates the two halves of the shared catalogue. Colour-emoji glyphs ignore SVG stroke / fill, so the theme tint is a natural no-op on them. This replaced the **Emoji category inside the Icons tab** (spec/85), which put colour art in a line-art catalogue that takes the theme colour.
...
| `E`       | Eraser                                                 |
| `P`       | Freehand pencil                                        |
| `6`       | Shape Pen (the same stroke, recognised — spec/115)     |
| `F`       | Frame                                                  |
