...
- **Database schema** _(Technical)_ — an entity-relationship (ER) diagram (kind id stays `er-diagram`): four entity tables (Users / Orders / Products / OrderItems) in a 2×2 grid, each an **Entity** element ([spec/120](120-entity.md)), wired by `1 : N` relationship arrows.
...
- **Class diagram** _(Technical)_ (kind id `uml-class`): a media-library model (abstract MediaItem, Song / Podcast subclasses, an aggregating Playlist). Each class is an **Entity** element ([spec/120](120-entity.md)): a title bar over member rows with `-` / `+` visibility markers; generalisation edges carry **hollow-triangle** arrowheads and the Playlist aggregation a **hollow diamond** + `0..*` label.
...
Groups were removed in [spec/146](146-web-components-and-no-groups.md): no `groupId`, no Group / Ungroup, no group mode, no drill-in, no `pinned-group` arrow ends. A multi-selection moves, styles, locks and deletes many elements at once, and a frame keeps a region's contents travelling together. Diagrams saved with groups load with the grouping dropped and any group-pinned arrow end frozen where it was (`migrateLegacyGroups`).
