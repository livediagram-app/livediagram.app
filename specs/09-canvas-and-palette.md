...
`TemplateDescriptor.extra?: boolean` is retained as catalogue metadata (10 default + 37 extra, pinned by `templates.test.ts`), but the picker no longer gates behind a "Show more" toggle: it browses by category (overview → drill-in, plus flat search) in a height-capped scroll area (see "Templates section" above).
