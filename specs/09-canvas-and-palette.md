...
**Sticky notes read as paper**: square corners (die-cut, never rounded — the canvas and the SVG export agree on `rx=0`), and the default shadow says the note is glued along its TOP STRIP with the rest of the sheet lifting ever so slightly off the board. **Nothing about the note itself changes** — same flat rectangle, same fill — and the shadow's silhouette is the note's own rectangle (a tilted or inset caster reads as the note warping, which is exactly wrong). `.lvd-sticky-peel` (globals.css) builds it from two pseudos: `::before` is a blurred solid copy of the rectangle nudged a few px down, vertically masked so it fades to nothing over the glued strip — a faint halo appears partway down the sides, strengthens continuously toward the bottom, and peaks under the bottom edge — and `::after` repaints the sheet over the caster's interior (negative-z pseudos paint above their element's background), leaving only the blur that escapes the edges, plus a whisper of pressed-down shading at the top strip. A user-set element shadow (spec/86) replaces the peel outright (the class is dropped with it): an explicit choice wins.
...
- **Event storming** _(Technical)_: the sticky-note workshop notation for exploring a business domain ([spec/139](139-event-storming.md)). Starts minimal by design: three gently tilted orange domain-event stickies (past tense, left to right in time order) with a muted method-reminder caption beneath; the full note grammar lives in the Write tab's **Event Storming** palette group, and the board ships four stage layers (a hidden Timeline-rail scaffold under Big picture / Process / Design) driven by the editor's workshop-view switcher — see spec/139. Orange is the notation, so the stickies carry an explicit `fillColor` (stickies are exempt from theme recolouring, so it survives every theme). Dot-grid backdrop like the other sticky workshop boards, not the technical graph paper.
