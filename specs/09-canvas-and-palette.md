...
The palette is laid out top-to-bottom as: canvas-tool toggle (Select / Hand / Laser / Eraser, a joined segmented control) → a **category tab bar**. The tabs render as a **joined segmented control** (one bordered group, dividers between, the active tab filled edge-to-edge) so it reads as "pick one"; each tab is an icon above a short text label (a triangle+circle+square cluster for Shapes, a wrench for Tools, a monitor for Devices, a smiley glyph for Icons). The categories are **Favourites** (a user-curated grid, listed first — see [spec/78](78-palette-favourites.md)), **Shapes**, **Tools**, **Devices**, **Icons**, **Technology**, **Components**, **Data**, and **Behaviour**, grouped in the picker into three named bands — **Common** / **Decorate** / **Interactive** — with Favourites full width above them and no band of its own (see [spec/110](110-palette-top-level-categories.md)). Every fixed creation tile (the Shapes / Tools / Behaviour / Data / Components / Devices tabs) is defined once in the shared catalogue `palette-tile-defs.tsx` and rendered through `PaletteTileGrid` or `PaletteToolRows` (spec/78), so a tile's glyph / behaviour can't drift between its home tab and Favourites. **Changing category is click-only**: clicking a tab expands its panel below, clicking the active tab again collapses it, clicking another switches. One tab is open at a time (mutually exclusive by construction), so the palette stays compact however many categories we add. There is **no hover preview** — hovering a tab never changes the open category (only a deliberate click does), so the panel never shifts underneath the pointer as it crosses the tab row. **Favourites is open by default** — the user's own go-to tiles (spec/78; it superseded Shapes as the default, which had been the most common entry point before Favourites existed). This replaced the earlier always-visible shape row plus a stack of one-per-category accordions (Tools / Devices / Icons), which didn't scale as categories grew. Switching categories is **softened**: the panel height eases off the measured content height (so a short category like Tools glides into a tall one like Icons instead of snapping) and the new content fades in. The height transition is gated on only after the first measured frame so the open-by-default panel doesn't animate itself open on load. The tab bar is a reusable, config-driven component (`apps/live/components/palette/PaletteTabBar.tsx`): it owns the active-tab state (and a `defaultOpenId` for the open-by-default tab) and takes a `tabs` array of `{ id, label, description, icon, content, group }`, so a new category is one more array entry in `CommandPalette.tsx` with no UI plumbing. The editor palette **always opens on Favourites** when a diagram loads — it deliberately does **not** persist the last-used category across diagrams (no `storageKey`), since the user's curated go-to grid is the right landing every time. (`PaletteTabBar` still supports an optional `storageKey` for other callers that want the choice remembered; a stale stored id falls back to the default.)

**Sub-categories keep the tab count down.** When a category holds distinct-but-related families, it splits them within the one panel rather than spending a whole top-level tab on each. Two shapes exist: a lightweight always-visible uppercase heading (`PaletteSectionLabel`, used by the Favourites edit groups) for a panel with only a couple of short sections, and — where the sections multiply — a **drill-in**. The **Tools** tab uses the drill-in: as the tool count grew past sixteen, a flat grid stopped scanning (a pencil, a table, and an avatar are not one family), and the stack of accordions it became was worse — seven collapsed headers meant no tool was visible until you opened one, every open group pushed the rest off the bottom, and you navigated a list of words in a palette that otherwise trades in pictures. So the tab opens on a **grid of short category tiles** (two columns, ~58px, the group's first tile as its artwork so a category always looks like what it holds, its contents described in the hover tooltip). Clicking one replaces the grid with that group's tools **as rows** — glyph, name, and a one-line description of what the tool is (`blurb` on the tile def, kept to about 35 characters; a test fails if a tile in any row-rendered section — Tools, Behaviour, Data, Components — lacks one or just repeats its caption). Rows rather than the 3-column grid every other category uses, because a palette is ~256px wide and three columns leaves room for a caption and nothing else: that works for Shapes and Icons, where the picture IS the explanation, and fails for Tools, where half the entries are behaviours whose glyph can't say what they do. Density is affordable here precisely because the drill-in means you arrive at a group of three to eight tools rather than all 28. The rows sit under a **breadcrumb bar** — `‹ Tools › Write & Draw`, on its own tinted surface with a full-height back target — so you always know where you are and can get back in one click. The groups: **Write & Draw** (Text, Note, Annotation, Page, and the gesture tools — Pencil, [Highlighter](81-highlighter.md), [Polygon](84-polygon-tool.md), Arrow), **Structure** (Table, Frame, [Timeline](51-timeline-rail.md)), **Blocks** (the rich content cards — [Code block](82-code-block.md), [Checklist](83-checklist.md), [Link card](40-link-cards.md)), and **People & Media** (Image, Avatar, User). **Behaviour** (the elements that DO something when interacted with — [Selection Mode button](103-mode-button.md), [Portal](104-portal-element.md), [Session button](105-session-button.md), [Reveal zone](106-reveal-zone.md), [Picker](107-picker.md)) and **Data** (the charts from [spec/53](53-pie-chart.md) — pie, bar, line, progress, rating) were groups here too, and are now top-level categories of their own ([spec/110](110-palette-top-level-categories.md)) — they keep their `toolGroup` tag, so the Tools tab subtracts them rather than a second list defining them. Group membership lives on the tile defs (`toolGroup` in `palette-tile-defs.tsx`, with the ordered `TOOL_GROUPS` catalogue and each group's one-line description beside it); tile **ids are unchanged**, so favourites persistence (spec/78) and the search panel are untouched by the grouping. Opening a category emits `UI`/`Opened`/`ToolGroup` telemetry (spec/22). **A search box sits above the navigation** (the same input + inline clear button as the Icons / Technology pickers, via the shared `PaletteSearchInput` primitive): drilling in keeps the tab compact but hides the other groups, so typing filters by tile label / caption / description across **every group still in this tab** and replaces the whole navigation with one flat grid of matches (with a "No tools match" empty state); clearing the query restores wherever you were. The first keystroke per mount emits `UI`/`Searched`/`ToolSearch` (spec/22).

**Palette tiles preview the active tab theme.** Picking a theme tints the palette so the tiles look like what they'll drop. The **boxed-shape tiles** — every Shapes tile, every Devices tile, and the Annotation tool — render as **filled mini-previews** in the theme's `elementFill` + `elementStroke`, mirroring how a new boxed element is coloured (`deriveNewBoxedColours`). The **line-art tools** (Text, Pencil, Arrow, Table, User, Frame, Polygon, Checklist) and the **Icons** grid **tint** their stroke to the theme's `elementStroke` (matching the on-canvas behaviour where an icon is tinted by the element's stroke colour). Tiles whose colours are fixed regardless of theme stay untinted: the **sticky note** (always amber), the **highlighter** (its yellow band is the tool's identity, spec/81), the **code block** (its dark card is its identity, spec/82), the **image** placeholder and **link card** (neutral chrome), and the full-colour **Technology** brand icons (native colour **emoji** in the Icons grid simply ignore the tint, spec/85). Under the **Basic** theme (`elementStroke` / `elementFill` are `null`) nothing is tinted and the palette keeps its default slate look. The pressed (queued draw-to-size) tile keeps its brand highlight so it still reads as selected. Mechanically: `CanvasChrome` derives a `themeTint` (`{ stroke, fill }`, undefined for Basic) from `getTheme(tabThemeId)` and passes it to `CommandPalette`, which exposes it via `PaletteTintProvider`; `IconButton` consumes the context and applies the stroke as the glyph's `color` (so every `stroke="currentColor"` SVG follows) plus, on `filled` tiles, a `--tile-fill` CSS variable that the `.palette-tile-filled` rule in `globals.css` paints into the otherwise-hollow `fill="none"` paths. Caption text stays on the neutral slate tone so labels keep their contrast on the app's light/dark panel surface (the palette panel does **not** adopt the canvas backdrop). Tiles opt out with `noTint` and into the filled preview with `filled`.
