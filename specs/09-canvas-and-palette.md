...
Groups were removed in [spec/147](147-web-components-and-no-groups.md): no `groupId`, no Group / Ungroup, no group mode, no drill-in, no `pinned-group` arrow ends. A multi-selection moves, styles, locks and deletes many elements at once, and a frame keeps a region's contents travelling together. Diagrams saved with groups load with the grouping dropped and any group-pinned arrow end frozen where it was (`migrateLegacyGroups`).
