...
**Sub-categories keep the tab count down.** When a category holds a couple of distinct-but-related families, it splits them into labelled **sub-sections** within the one panel rather than spending a whole top-level tab on each — a lightweight uppercase heading (`PaletteSectionLabel` in `palette-create-tabs.tsx`), **not** an accordion: every section stays visible, nothing collapses. The **Tools** tab uses this: as the tool count grew to sixteen, the old flat **Tools** grid stopped scanning (a pencil, a table, and an avatar are not one family), so the tab is grouped into themed sub-sections, each a labelled 3-column grid: **Write** (Text, Note, Annotation), **Draw** (the gesture tools — Pencil, [Highlighter](81-highlighter.md), [Polygon](84-polygon-tool.md), Arrow), **Structure** (Table, Frame, [Timeline](51-timeline-rail.md)), **Blocks** (the rich content cards — [Code block](82-code-block.md), [Checklist](83-checklist.md), [Link card](40-link-cards.md)), **People & media** (Image, Avatar, User), and finally the pre-existing **Data** section (the charts from [spec/53](53-pie-chart.md) — pie, bar, line, progress, rating), folded in from what used to be a standalone Data category. Group membership lives on the tile defs (`toolGroup` in `palette-tile-defs.tsx`, with the ordered `TOOL_GROUPS` catalogue beside it); tile **ids are unchanged**, so favourites persistence (spec/78) and the search panel are untouched by the grouping.
