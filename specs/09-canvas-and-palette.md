...
**Sub-categories keep the tab count down.** When a category holds a couple of distinct-but-related families, it splits them into labelled **sub-sections** within the one panel rather than spending a whole top-level tab on each — a lightweight uppercase heading (`PaletteSectionLabel` in `palette-create-tabs.tsx`), **not** an accordion: every section stays visible, nothing collapses. The **Tools** tab uses this: a **Tools** section (the drawing / element tools below, plus the [Highlighter](81-highlighter.md), the [Polygon tool](84-polygon-tool.md), the [Code block](82-code-block.md), and the [Checklist](83-checklist.md)) followed by a **Data** section (the charts from [spec/53](53-pie-chart.md) — pie, bar, line, progress, rating), folded in from what used to be a standalone Data category.
