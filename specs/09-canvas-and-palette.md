...
- **Milestone timeline** _(Project Management)_: the richer, presentation-ready sibling of the default Timeline, designed from scratch rather than derived from it. A directional spine (arrowhead pointing forward in time) carries five milestone dots; each hangs a rounded card off a **pinned stem** (dragging a card keeps its stem attached), alternating above and below, with a **date chip** riding the stem near the spine and a muted one-line description beyond the card. Kick-off is soft-tinted as the entry point and Launch is the bold hero; a bold plan title ("Launch plan · 2027") anchors the top-left. Ships the Timeline's "Spine" / "Milestones" layer split (spec/74) and the `lines` backdrop.
...
`TemplateDescriptor.extra?: boolean` is retained as catalogue metadata (10 default + 35 extra, pinned by `templates.test.ts`), but the picker no longer gates behind a "Show more" toggle: it browses by category (overview → drill-in, plus flat search) in a height-capped scroll area (see "Templates section" above).
