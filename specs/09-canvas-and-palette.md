...
- **Event storming** _(Technical)_: the sticky-note workshop notation for exploring a business domain ([spec/139](139-event-storming.md)). Starts minimal by design: three gently tilted orange domain-event stickies (past tense, left to right in time order) with a muted method-reminder caption beneath; the full note grammar lives in the top-level **Event Storming** palette category (Structure band), and the board ships three stage layers (Big picture / Process / Design) driven by the editor's workshop-view switcher — see spec/139. Orange is the notation, so the stickies carry an explicit `fillColor` (stickies are exempt from theme recolouring, so it survives every theme). Dot-grid backdrop like the other sticky workshop boards, not the technical graph paper.
