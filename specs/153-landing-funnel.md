...
spec/150 already counts, that gives a three-step funnel per page and per
button:

1. **Viewed**: the page was viewed (`Page·View·<path>`, spec/150).
...
from a comparison page?" had no answer. spec/16's landing page work (a new
...
So the marketing site still sends **only page views** (spec/150): no click
...
three independent counts divided by each other, exactly as spec/22 requires.
...
The Pages tab (spec/150) opens with a **Landing Funnel** section above the top
