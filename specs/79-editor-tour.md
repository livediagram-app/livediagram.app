...
- **The welcome card also picks the panel layout.** Under its copy sit
  ...
  Toolbar, [spec/148](148-toolbar-layout.md)), one button each, the one in
  ...
  Floating rings Toolbar there, which is what the phone shows, spec/148),
  the write, and
...
- **Layout pick** on the welcome card: the Settings row's own
  `'UI'/'Changed'/'PanelLayout<Option>'` (spec/148), fired before the
  write.
