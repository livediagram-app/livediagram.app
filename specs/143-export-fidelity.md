...
([spec/54](54-live-image-share.md)), and the inline images the MCP server
returns ([spec/62](62-mcp-server.md)).
...
- **Motion.** Every element animation is designed so its resting frame is the
  static element ([spec/09](09-canvas-and-palette.md)), which is what a still
  image wants.
...
- **Live state of a control.** A timer's remaining time, a picker mid-roll, a
  pressed button.
- **The Collaborate paper kit** ([spec/122](122-participant-responses.md)): the
