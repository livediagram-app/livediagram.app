...
- Per-worker secrets that are **not** in the optional sync table above (e.g. `AI_API_KEY`, `RESEND_API_KEY`) — those are set by hand with `wrangler secret put` and persist across deploys.
