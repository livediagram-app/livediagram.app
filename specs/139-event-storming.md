...
| **Adjacent lane**                    | EDGES LINE UP. Same silhouette: left edge to left edge, and for two SQUARES also the brick (centred on the gap beside the note). Different silhouettes: left edge to left edge, or right edge to right edge — and no brick, because there is no sensible brick between a square and a wide note.                                                 |
