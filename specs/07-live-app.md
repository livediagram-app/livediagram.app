...
- Preference persists in `localStorage` under `livediagram:v2:ui-mode` (values `'light'` / `'dark'`), across three small modules with one job each. `hooks/ui/ui-mode-storage.ts` holds just the key, as a PLAIN module so the server layout can inline it (a client boundary there hands the layout a stub, which broke the pre-hydration script once). `hooks/ui/ui-mode-store.ts` holds the value, the read, the write and the `.dark` class on `documentElement` — no React, so it is directly testable. `hooks/ui/useUiMode.ts` is the subscription and the toggle.
- The choice is applied on **every** route by a tiny inline script in the root layout (`app/layout.tsx`) that runs before first paint. This matters for two reasons. React mounts after first paint, so ANY hook-based application flashes light before it lands. And the hook is not mounted everywhere anyway — the TabBar, the theme-match nudge, the mode toggle and the theme browser render it, but the standalone `/new` route renders none of them, so without the layout script it would paint light over a dark body and stay that way. The script reads the same localStorage key and adds `.dark` synchronously, so there's no light-mode flash on any surface.
