...
(spec/68 §1), and the Assign Action dialog, the popover, complete / reopen,
the Collaborate panel's rows, the Activity page (spec/142), the assignment
...
- **Delete** is not on the face. Deleting the card deletes its action with it
  (spec/68 §3), and the element menu's Collaborate → View Action popover keeps
...
- **Read-only** surfaces (a view-role visitor, the embed, a presentation) show
  the card readable but with no buttons, like the popover (spec/68 §7). An
...
exactly as spec/136 does it, and for the same reasons: "about" is what an arrow
...
- Not self-painting: a card that wants the fill, border and rounded corners
  ...
  allow-by-default.
- **Not votable** (spec/39): it is a task, not a candidate.
...
- Telemetry: placing it is the ordinary element-created event with type
  `ActionPanel`; everything done from the card fires spec/68's existing
  `Action` events.

## Not in scope

- More than one action per card. One action per element is spec/68's rule,
