...
- **Canvas primitives**: shapes (fourteen geometric primitives plus frames, device mockups for wireframing, progress bars / rings, star ratings, timeline rails, pie / bar / line charts, syntax-highlighted code blocks, and tickable checklists, plus a curated single-colour icon catalogue with an emoji section), tables, sticky notes, text, images, arrows (straight / curved / angled with draggable curve / elbow handles, configurable thickness + arrowhead size, optional labels), freehand sketches via the Pencil and translucent Highlighter tools (smooth SVG paths with optional shape-recognition mode that auto-converts rectangles / circles / diamonds / lines), a click-to-place polygon / polyline tool, comment threads, per-element links + locks, and drop shadows with presets + sliders (see [spec/86](../specs/86-element-shadows.md)).
