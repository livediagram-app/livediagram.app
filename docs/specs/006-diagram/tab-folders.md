...
- **Per-diagram.** Tab order already lives on the `diagram_tabs` link, not on the shared `tabs` row, because a tab can be shared into several diagrams ([Tab ↔ diagram many-to-many](tab-diagram-many-to-many.md)). Folder membership lives in the same place, for the same reason: a tab shared into two diagrams can sit in a folder in one and be loose in the other. Folder membership is **never** part of the tab body (`tabs.data`).
...
Migration `0018` adds one nullable column to the link table from [Tab ↔ diagram many-to-many](tab-diagram-many-to-many.md):
...
Reuses the closed vocabulary ([Telemetry + public transparency dashboard](../017-telemetry/telemetry.md)) with **the subject deciding the category**, and every event carries a `type` so none of it collides with tab lifecycle:
...
- [Tab ↔ diagram many-to-many](tab-diagram-many-to-many.md) — the `diagram_tabs` link table folder now extends.
