...
Tab bodies are stamped with their **board kind** on the way into IndexedDB (`upsertTab`), exactly as the cloud path stamps them in `tabForWire` — the two stores must agree about what a tab IS, or a Sync Diagram would hand the cloud a board that has forgotten itself ([Event storming](../021-event-storming/event-storming.md)).

## Turning it on

Offline Mode is **off by default**. You choose it when creating a diagram:

- The **New Diagram** wizard ([Dedicated route for new-diagram creation](../007-editor/new-diagram-route.md)) runs three steps: Template, Theme, then
  **Location** (the Settings step in code). It carries the **Save location** chooser
  ([Save Locations](save-locations.md)), alongside the diagram name and where it is saved (a personal
...
The diagram model (`Tab[]` + meta, per [Diagram structure](diagram-structure.md)) is **identical** — an offline
...
| ----------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Share links / embeds                                                    | The header **Share** button stays visible, but for an offline diagram it opens a gate prompting you to sync to your account first (nothing lives on a server to share yet). Read-only embeds are unavailable for the same reason ([Live app](../007-editor/live-app.md), /54). |
| Live presence / realtime                                                | N/A — a private diagram never opens the room anyway ([Realtime conflict resolution](../012-collaboration/realtime-conflict-resolution.md)).                                                                                                                                    |
...
| AI assistance                                                           | Works online — it reads the current canvas, not the stored diagram ([AI Assistance](../007-editor/ai-assistance.md)); only a lost connection stops it.                                                                                                                         |
...
| Tab linking ([Tab ↔ diagram many-to-many](tab-diagram-many-to-many.md)) | Unavailable in either direction — linked tabs are one shared server row.                                                                                                                                                                                                       |
...
**Images embed locally ([Image element + per-owner gallery](../009-elements/images.md)).** In a cloud diagram, an added image uploads
...
Autosave still runs for an offline diagram ([Per-tab storage](per-tab-storage.md)) — it writes to IndexedDB
...
- Uploads the diagram's meta + tabs to the API ([API app](../015-api/api.md)), creating a normal
...
- It differs from a **guest cloud** diagram ([Auth + guest access](../014-identity/auth-and-guest-access.md)): a guest's cloud diagram is
...
- Sign-in / sign-out never migrates offline diagrams (the migrate flow, [Auth + guest access](../014-identity/auth-and-guest-access.md),
...
Also add the standard contextual help link ([Contextual help links](../018-help/contextual-help-links.md)) from the New Diagram
...
- No diagram content or ids ever leave (the `type` bound in [Telemetry + public transparency dashboard](../017-telemetry/telemetry.md) holds).
...
- New Diagram wizard (`TemplatePicker` / `template-picker-settings.tsx`,
  [Dedicated route for new-diagram creation](../007-editor/new-diagram-route.md)): the **Settings** step's Save location chooser ([Save Locations](save-locations.md), the
...
[Diagram structure](diagram-structure.md) (diagram model),
[API app](../015-api/api.md) (api + persistence),
[Per-tab storage](per-tab-storage.md) (per-tab storage + autosave),
[Dedicated route for new-diagram creation](../007-editor/new-diagram-route.md) (New Diagram),
