...
- Foundation for "this tab is shared across diagrams" reuse — landed in [Tab ↔ diagram many-to-many](tab-diagram-many-to-many.md) (migration 0011 / item #13). The link table sits over the same tab body rows described here.
...
- Sharing (per-diagram share codes; no per-tab sharing yet). Tab-level reuse across diagrams is a server-side relationship under [Tab ↔ diagram many-to-many](tab-diagram-many-to-many.md), not user-visible sharing.
