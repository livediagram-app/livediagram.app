...
- **Documenting the realtime WebSocket protocol** (`GET /api/diagrams/:id/ws` and the Durable Object op messages). OpenAPI describes the REST surface; the WS upgrade is listed as an endpoint but its message protocol stays in [API app](api.md) prose.
