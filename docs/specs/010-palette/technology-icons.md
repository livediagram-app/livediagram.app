...
The Icons catalogue (`apps/live/lib/icons.ts`, [Canvas and palette](../008-canvas/canvas-and-palette.md)) is Feather/Lucide-style:
...
- **The heavy data loads async.** `tech-icons.ts` is the synchronous API
  ...
  editor's first-load JS (see [Canvas and palette](../008-canvas/canvas-and-palette.md) "Catalogue loading"). `isTechIconId` —
...
The **System architecture** starter ([Canvas and palette](../008-canvas/canvas-and-palette.md) "Templates",
...
category/action pair ([Telemetry + public transparency dashboard](../017-telemetry/telemetry.md)). The `type` is the constant `'TechIcon'`, never
