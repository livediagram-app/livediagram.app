...
- An **Edit** affordance (absent in the Toolbar layout's More popover, which
  orders Favourites by use, [Toolbar layout](../007-editor/toolbar-layout.md)) renders as a full-width **footer band** at the
...
- **Store** (`lib/palette-favourites.ts`): pure load / add / remove helpers
  ...
  JSON fallback).
- Telemetry ([Telemetry + public transparency dashboard](../017-telemetry/telemetry.md)): `track('UI', 'Added', 'PaletteFavourite')` /
