...
Users can build their own **themes**, save them to their account, reuse them across diagrams, and edit them later. A custom theme is the same shape as a built-in one ([Canvas + Theme dialog](canvas-and-theme-dialog.md), [Multi-colour (rainbow) themes](multicolour-themes.md)), so once saved it behaves exactly like Forest or UML: pick it on a tab and new elements adopt its colours.
...
- Custom themes are **owner-scoped**, exactly like diagrams and folders ([API app](../015-api/api.md)): keyed by the request owner id, which is the Clerk `sub` for signed-in users or the `X-Owner-Id` guest id otherwise ([Auth + guest access](../014-identity/auth-and-guest-access.md)). **Guests get them too** — consistent with the canvas-works-without-sign-in principle — and a guest who signs up carries them over via the existing `POST /api/migrate` owner-id remap.
...
The Theme tab of the Tab Look & Feel dialog ([Canvas + Theme dialog](canvas-and-theme-dialog.md)) gains:
...
The Explorer ([Folders](../013-workspace/folders.md)) gains a **Themes** entry under its **Library** section (beside Image gallery):
...
New `type`s on the existing `Theme` category ([Telemetry + public transparency dashboard](../017-telemetry/telemetry.md)): `track('Theme', 'Created', 'Custom')` on save-new, `track('Theme', 'Changed', 'CustomEdited')` on edit-save, `track('Theme', 'Deleted', 'Custom')` on delete. Applying a custom theme reuses the existing theme-pick event.
