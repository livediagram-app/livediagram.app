# Read-only embeds (`/embed`)

Tracked as [issue #8](https://github.com/livediagram-app/livediagram.app/issues/8). [Purpose](../001-project-vision/purpose.md) says "diagrams may be embedded elsewhere"; this spec makes a shared diagram iframe-able in Notion, wikis, and docs. Every embedded diagram is a distribution channel for a free product whose growth model is distribution ([Open source + distribution](../002-project-scope/open-source-and-business-model.md)).
...
- **Hides all chrome**: header, TabBar, every floating panel (the same gates zen mode uses, [Zen mode](../007-editor/zen-mode.md)). What remains: the canvas (pan / zoom / pinch), the ZoomControls dock (without the zen toggle), and the two embed affordances below.
