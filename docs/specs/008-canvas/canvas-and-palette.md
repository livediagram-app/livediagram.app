...
```ts
// The sixteen anchors: see arrow-anchors.md.
type Anchor =
  | 'n'
  | 'nne'
  | 'ne'
...
- Anchors at element centres or at free positions along an edge.
...
- **Arrow keys** nudge the selection 1 px (10 px with `⇧`); on an event-storming board, up / down move a workshop note a whole lane ([Event storming](../021-event-storming/event-storming.md#always-on-a-lane)).
...
**Fit-to-screen** centres the viewport on the active tab's content and picks the largest zoom level that fits every element with a small margin. Empty tabs reset to (0, 0) at zoom 1. Same gesture as Figma / Excalidraw's "Zoom to fit". Also fires automatically **once per tab entry, when the tab's content has loaded** (on diagram open and again on each tab switch into a non-empty tab) so a saved tab loads framed rather than at the previous session's pan / zoom. An entry onto an empty tab is spent without fitting, so what the user then adds, the **first element included, stays exactly where it was placed**: the view never jumps to recentre it. Replacing a tab's content wholesale, by picking a template or importing, frames the new content (`requestFit` in `useTabEntryEffects`); the Blank template leaves the view alone.
...
- **Arrow anchoring is rotation-aware and outline-aware**: anchors rotate with the element and sit on its drawn outline; see [Arrow anchors and auto-rebind](arrow-anchors.md) "Anchor geometry", which also owns the anchor chosen at creation and the auto-rebind.
...
- **Keyboard nudging** — with a selection (single or multi), the **arrow keys** move it by **1 px** per press, or **10 px** with **Shift** held. Boxed elements shift their `x`/`y`; free arrow endpoints shift their `from`/`to`; pinned arrow ends follow their anchored element and (when the `autoRebindArrows` preference is on, see [User preferences](../007-editor/user-preferences.md)) run the auto-rebind (`rebindArrowAnchorsAfterMove`, [Arrow anchors and auto-rebind](arrow-anchors.md)), exactly as a drag-move does. A run of nudges coalesces into a **single undo step**: the first press takes a history checkpoint, subsequent presses `tick` the present without pushing history, and the burst closes after a short idle. Suppressed while editing a label or with focus in a text input; view-role sessions don't nudge. **On an event-storming board** a selection holding a workshop note moves a whole LANE per up / down press (Shift or not), every member by that note's step, so the notes stay on their lanes; left / right are unchanged ([Event storming](../021-event-storming/event-storming.md#always-on-a-lane)). Telemetry: `track('Element', 'Changed', 'Nudge')` once per burst.
...
drops the copies 24px offset on the active tab. On an event-storming board a paste holding a workshop note lands at the POINTER when the pointer is over the canvas, and otherwise staggers on the original along its lane (24px right, same lane); either way every workshop note lands on a lane ([Event storming](../021-event-storming/event-storming.md#always-on-a-lane)). `⌘X` is the same copy followed
