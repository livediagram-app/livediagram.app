...
Per [Telemetry + public transparency dashboard](../017-telemetry/telemetry.md): changing a setting emits `UI·Changed·LaserWidth` / `LaserColour` / `LaserTrail` / `LaserEffect`. The colour VALUE is a preset token, never a hex the user typed — there is no custom colour here, deliberately: it would be one more thing to get wrong mid-presentation.
