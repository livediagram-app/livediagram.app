...
While the **Format** painter ([Canvas and palette](canvas-and-palette.md)) is active, a **Format** panel is present: which parts of the copied style actually get painted, and whether the brush stays loaded after a paint. The fifth mode panel, after [Avatar](avatar-mode.md), [Laser](laser-panel.md), [Spotlight](spotlight-panel.md), and [Eraser](eraser-panel.md).
...
- **Copies** — five toggles, all on by default (which is exactly today's painter):
  - **Fill** — the fill colour, a table header's or lane gutter's fill, its theme-preset binding ([Style presets](../010-palette/style-presets.md)), and the fill's theme lock.
  ...
  - **Effects** — shadow ([Element shadows](element-shadows.md)), opacity, and the looping / icon / flow animations.
...
Device-local, in `localStorage` (`livediagram:v2:format-config`), like the other tool panels. Never sent to the api, never in the synced preferences blob ([User preferences](../007-editor/user-preferences.md)).
...
Per [Telemetry + public transparency dashboard](../017-telemetry/telemetry.md): toggling emits `UI·Changed·FormatCopies`, and the mode emits `UI·Changed·FormatMode`. Painting itself still reports `Element·Changed·FormatPainter`, as before.
...
- Saving named brushes ("my heading style"). That is style presets ([Style presets](../010-palette/style-presets.md)) wearing a different hat.
