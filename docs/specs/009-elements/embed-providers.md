...
[YouTube video element](youtube-video.md) said the parser, the poster URL and the embed origin were the only
...
The video's restricted picker ([YouTube video element](youtube-video.md)) now validates with `embedTargetFor`
...
It became necessary when Behaviour grew two groups ([Session button](../012-collaboration/session-button.md)'s session tools
and [The Reaction Pad](reaction-pad.md)'s reactions) holding eight tiles between them. With both open the
