...
- A **shape kind**, `mode-button`, rather than a new element type — exactly the call [The Page element](page-element.md) made for the Page. It inherits everything a shape already has: label, colours, border, shadow, resize, rotation, layers, groups, comments, actions, export.
