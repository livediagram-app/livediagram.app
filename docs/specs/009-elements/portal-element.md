...
- A **shape kind**, `portal`, like the Selection Mode button ([Selection Mode button](mode-button.md)) and the Page ([The Page element](page-element.md)) — so it inherits colours, shadow, layers, groups, comments, export.
...
Per [Telemetry + public transparency dashboard](../017-telemetry/telemetry.md): adding one emits `Element·Added·Portal`; linking / unlinking / renaming emits `Element·Changed·Portal`. Travelling is not tracked — it is navigation, the same as panning, and would be the chattiest event in the app.
