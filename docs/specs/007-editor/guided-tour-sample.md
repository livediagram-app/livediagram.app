# Guided tour sample ("Take the guided tour") — RETIRED

> **Retired, July 2026.** The interactive editor tour
> ([Interactive editor tour ("Show me around")](editor-tour.md)) superseded this sample: the annotated
...
whose annotation markers ([Annotations](../009-elements/annotations.md)) explain the core
...
2. **Editing Elements**: rename / rotate demos, status shape markers
   ([Shape markers](../009-elements/shape-markers.md)), border styles, and the data shapes (progress bar + ring, star
   rating).
...
Each section carries **one annotation marker** ([Annotations](../009-elements/annotations.md)) with the deeper
...
paths (the standard `titleCaseType(kind)`, see [Telemetry + public transparency dashboard](../017-telemetry/telemetry.md)).
