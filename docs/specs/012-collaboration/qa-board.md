...
A **shape kind**, `qa-board`, in the Collaborate family ([Per-participant responses](participant-responses.md)). Its `label`
...
- **It publishes nothing.** The owner id is a credential ([Per-participant responses](participant-responses.md)), and a
...
- Still inside the step, the room broadcasts
  ...
  reconnects within the log window replays it ([Realtime conflict resolution](realtime-conflict-resolution.md)).
- The response returns the same `{ notes, rev }`.

### Not repeating the dot vote's bug

The dot vote ([Session tools (timer + voting)](session-tools.md)) once lost votes because a peer's write carried a
...
its contents to the box ([Per-participant responses](participant-responses.md)), because its content is fixed and resizing
