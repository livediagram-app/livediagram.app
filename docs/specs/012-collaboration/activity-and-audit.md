...
2. `useDiagramHistory.undo` / `redo` are paired with the activity log
   ...
   - Emits whose mutation doesn't push history ([Session tools (timer + voting)](session-tools.md) session tools)
...
a Square to a Circle`. A style-preset apply ([Style presets](../010-palette/style-presets.md)) is one look
...
- **Hovering a revertable row live-previews its revert.** Resting a
  ...
  tick + autosave guard as the [Style presets](../010-palette/style-presets.md) style previews; see
  ...
  ([User preferences](../007-editor/user-preferences.md), Panels > Activity), backed by the synced
