# Keeping a poll's results

Status: **implemented**.

When the host ends a live poll ([Live poll (ephemeral pulse-check)](live-poll.md)), they can keep the tallies as a chart
...
### A token poll → a bar chart ([Data charts (pie + bar + line)](../009-elements/pie-chart.md))
...
### A free-text poll → an opened idea box ([Idea box](idea-box.md))
...
nowhere to record an author ([Idea box](idea-box.md)) — and they can be scattered to sticky
...
- Late joiners are still not prompted, a reload still drops your answer, and
  the poll still dies with the last connected client ([Live poll (ephemeral pulse-check)](live-poll.md)). This adds one
