...
lives in `plans/0003-event-storming.md`.
...
- **Palette home:** a **top-level Event Storming category** in the
  Structure band ([Palette top-level categories and bands](../010-palette/palette-top-level-categories.md)) — promoted out of the Write accordion once
...
- **Notes are written in MARKER.** `ES_NOTE_FONT` is `permanent-marker`
  ([Fonts](../004-interface-design/fonts.md)'s catalogue), resolved from the note rather than stamped on the
...
- **Telemetry:** adds report `Element / Added / Sticky`, unchanged — the
  ...
  reasoning as Video's provider, [Telemetry + public transparency dashboard](../017-telemetry/telemetry.md)).
...
- **One size for life.** On an event-storming board every sticky drops
  ...
  with the [Selection Mode button](../009-elements/mode-button.md) fixed-size shape kinds): no resize handles, union
...
- **No palette chrome.** An ES board hides the palette's whole header
  ...
  by keyboard shortcut and the command palette ([Command palette (⌘K)](../007-editor/command-palette.md)).
...
- **Why Alt and not Ctrl or Shift.** Both are taken during a canvas
  drag: Cmd/Ctrl means free placement ([Snap override (free drag)](../008-canvas/snap-override.md)) and Shift means
  drag-duplicate ([Shift-drag duplicate](../008-canvas/shift-drag-duplicate.md)). Alt was unclaimed, so insertion displaces
...
- **One placement rule at a time.** While a slot is open the alignment
  ...
  disagree — the same discipline [Palette drag ghost + drop preview](../010-palette/palette-drag-ghost.md)'s ghost already follows. The
...
- **Precedence** (top rung wins): an open insertion slot (Alt, Phase 5) → free
  ...
  axis is left. Shift-duplicate ([Shift-drag duplicate](../008-canvas/shift-drag-duplicate.md))
...
- **One snap computation per gesture.** The palette path and the note-drag path
  ...
  that answer. A preview that disagrees with the drop is a lie ([Palette drag ghost + drop preview](../010-palette/palette-drag-ghost.md)).
...
- **The buttons earn their place.** At most two, only on the SELECTED note
  ...
  before this domain event"). [Event storming](event-storming.md) retired the four quick-connect pluses on
...
- **A failover is reported as a warning.** Each one sends one event to the
  ...
  ([Telemetry + public transparency dashboard](../017-telemetry/telemetry.md)): closed values, never a crop, a word or a count of notes. It is a
...
The plans live in `plans/0010-event-storming-photo-review.md`,
`plans/0011-event-storming-photo-surface-and-shade.md` and
`plans/0009-event-storming-photo-precision.md`.
...
- Hiding chrome has a second audience: the editor tour ([Interactive editor tour ("Show me around")](../007-editor/editor-tour.md))
