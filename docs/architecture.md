...
- **Realtime**: Cloudflare Durable Objects, one room per diagram. Edits sync as granular, id-addressed element ops, so concurrent edits to _different_ elements merge instead of clobbering, and the room stamps each mutation with a per-`epoch` sequence so peers converge and a reconnecting client catches up from a bounded op log (see [spec/75](../specs/75-realtime-conflict-resolution.md), levels 0–1). A full Yjs-CRDT path (field-level merge of the _same_ element) is implemented behind the `?yjs=1` flag, off by default (spec/75, level 2).
