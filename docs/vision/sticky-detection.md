...
2. **Classify each pixel** (`classifyRgb`) into a note kind, `wall` or `ink`,
   ...
   Blue and pink each come in two papers on the calibration walls, a vivid
   one and a pale one (periwinkle, pale pink). They are one kind but not one
...
| `CLOSE_NOTE_FRACTION`                       | 0.02              | The close repairs handwriting INSIDE a note, so it is sized from the note, capped at 0.6% of the frame. One pixel at 1000px: two welds flush notes.                                                     |
...
| `STEP_MIN_SPAN`                             | 1.6               | Across a box this many notes long a brightness step (one note's level a little under its neighbour's) is a seam. 1.4–1.65 score alike; at 1.7 the whiteboard's pairs, 1.68–1.74 notes long, are missed. |
