...
import { cornerForFace, faceOf, planEndpointRebinds, type EndPlan } from './arrow-rebind-plan';
...
export function rebindArrowAnchorsAfterMove(
  ...
  // Phase 1 — which endpoints need re-anchoring, and where each wants to go.
  const { plans, reassigning } = planEndpointRebinds(elements, includes, byId);
