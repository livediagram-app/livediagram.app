...
function isValidEndpoint(ep: unknown): boolean {
  ...
  if (ep['kind'] === 'pinned')
    return isNonEmptyStr(ep['elementId']) && ANCHORS.has(ep['anchor'] as string);
...
export function isValidElement(el: unknown): el is Element {
  ...
  if (t === 'shape') {
    ...
    if (el['railLabels'] !== undefined && !boundedArray(el['railLabels'], MAX_DATA_ARRAY))
      return false;
    if (el['lineCategories'] !== undefined && !boundedArray(el['lineCategories'], MAX_DATA_ARRAY))
      return false;
    if (el['pieSlices'] !== undefined && !boundedArray(el['pieSlices'], MAX_DATA_ARRAY))
      return false;
    if (el['lineSeries'] !== undefined && !boundedArray(el['lineSeries'], MAX_DATA_ARRAY))
      return false;
    ...
    if (
      el['code'] !== undefined &&
      (typeof el['code'] !== 'string' || el['code'].length > CODE_MAX_LENGTH)
    )
      return false;
    if (
    ...
    if (el['entityFields'] !== undefined) {
      ...
      for (const f of el['entityFields']) {
        ...
        if (
          f['type'] !== undefined &&
          (typeof f['type'] !== 'string' || f['type'].length > ENTITY_MAX_TEXT)
        )
          return false;
      }
    }
    ...
    if (el['agendaItems'] !== undefined) {
      ...
      for (const item of el['agendaItems']) {
        ...
        if (typeof item['label'] !== 'string' || item['label'].length > AGENDA_MAX_TEXT)
          return false;
    ...
    if (el['rollCall'] !== undefined) {
      ...
      for (const entry of el['rollCall']) {
        ...
        if (typeof entry['name'] !== 'string' || entry['name'].length > ROLL_CALL_MAX_TEXT)
          return false;
    ...
    if (el['checklistItems'] !== undefined) {
      ...
      for (const item of el['checklistItems']) {
        if (!isObj(item)) return false;
        if (typeof item['text'] !== 'string' || item['text'].length > CHECKLIST_MAX_TEXT)
          return false;
  ...
  if (t === 'freehand') {
    ...
    if (
      ...
      (!isNum(el['penWidth']) || el['penWidth'] < 1 || el['penWidth'] > 100)
    )
      return false;
